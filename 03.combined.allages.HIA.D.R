...
# ==========================================================================================================================================
  ...
  for (j in 1:length(cats)){
    ...
    years.popx <- years.pop[k]
      ...
      df5 <- subset(df4, year %in% years.bdrx)
      ...
      for (x in 1:nrow(concs2)){
         if(concs2[x,4]>0){
         concs2[x,34] <- mrbrt[,1][concs2[x,4] > mrbrt[,2] & concs2[x,4] < mrbrt[,3]]
         concs2[x,35] <- mrbrt[,1][concs2[x,32] > mrbrt[,2] & concs2[x,32] < mrbrt[,3]]
         }else{
           concs2[x,34] <- 0
           concs2[x,35] <- 0
         }
      ...
      for (x in 1:nrow(concs2)){
        ...
        if(concs2[x,4] < 10 & concs2[x,4] > 0){
          concs2[x,36] <- mrbrt[,1][concs2[x,4] > mrbrt[,2] & concs2[x,4] < mrbrt[,3]]
        }else{
          concs2[x,36] <- mrbrt[,1][mrbrt[,2] == 10.00]
        }
      } 
      
      colnames(concs2)[34] <- 'rr.oap'
      colnames(concs2)[35] <- 'rr.hap'
      colnames(concs2)[36] <- 'rr.who'
      ...
      df9$ac.hap <- df9$paf.pm.hap*df9$pop.sum.y*df9$Rate*10^-5 # using HAP
      df9$ac.nohap <- df9$paf.nohap*df9$pop.sum.y*df9$Rate*10^-5 # not using HAP
      df9$ac.who <- df9$paf.who*df9$pop.sum.y*df9$Rate*10^-5 # not using HAP
      
      df9<-df9[complete.cases(df9$popw), ]
      df9$anal.names <- paste(anal.names[k])

    # Write out new raster
    fout_1 <- paste('/GWSPH/groups/anenberggrp/VAS/GBD_2020/final/results/city_level/dem/',cats[j],years[k],'.csv',sep='')
...
setwd('/GWSPH/groups/anenberggrp/VAS/GBD_2020/final/results/city_level/dem/')

filenames <- list.files(path='/GWSPH/groups/anenberggrp/VAS/GBD_2020/final/results/city_level/dem/',pattern="*.csv", full.names=TRUE)
dataset <- do.call("rbind",lapply(filenames,FUN=function(files){read.csv(files, header=TRUE, sep=",")}))
write.csv(dataset, "combined.city.paf.dem.csv")
