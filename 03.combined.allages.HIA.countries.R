...
# ==========================================================================================================================================
  ...
  for (j in 1:length(cats)){
    ...
    years.x <- years[k]
      ...
      concs2$sum.pm <- concs2$mean.x + concs2$mean.hap   
      ...
      df9$ac.hap <- df9$paf.pm.hap*df9$population*df9$Rate*10^-5 # using HAP
      df9$ac.nohap <- df9$paf.nohap*df9$population*df9$Rate*10^-5 # not using HAP
      df9$ac.who <- df9$paf.who*df9$population*df9$Rate*10^-5 # not using HAP
      
      df9$per.diff.hap.who <- round((df9$ac.hap-df9$ac.who)/df9$ac.hap,4)
      df9$per.diff.nohap.who <- round((df9$ac.nohap-df9$ac.who)/df9$ac.nohap,4)
      
      df9$acrate.hap <- round((df9$ac.hap*100000)/df9$population,4)
      df9$acrate.nohap <- round((df9$ac.nohap*100000)/df9$population,4)
      df9$acrate.who <- round((df9$ac.who*100000)/df9$population,4)
      
      df9$per.diff.hap.who.rate <- round((df9$acrate.hap-df9$acrate.who)/df9$acrate.hap,4)
      df9$per.diff.nohap.who.rate <- round((df9$acrate.nohap-df9$acrate.who)/df9$acrate.nohap,4)
      
    # Write out new raster
    fout_1 <- paste('/GWSPH/groups/anenberggrp/VAS/GBD_2020/final/results/country_level/',cats[j],years[k],'.csv',sep='')
...
filenames <- list.files(path='/GWSPH/groups/anenberggrp/VAS/GBD_2020/final/results/country_level/',pattern="*.csv", full.names=TRUE)
dataset <- do.call("rbind",lapply(filenames,FUN=function(files){read.csv(files, header=TRUE, sep=",")}))
write.csv(dataset, "combined.country.paf.csv")
