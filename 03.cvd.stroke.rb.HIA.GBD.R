...
conc <- read.csv(paste('/GWSPH/groups/anenberggrp/VAS/GBD_2020/final/lookup/combined.hdc.GBD.expsum.csv',sep=''))
print('conc')
  ...
  for (k in 1:length(years)){
    ...
    for (i in 1:length(ages)){
      ...
      for (x in 1:nrow(concs2)){
        if(concs2[x,3]>0){
          concs2[x,33] <- mrbrt[,1][concs2[x,3] > mrbrt[,2] & concs2[x,3] < mrbrt[,3]]
      ...
      for (x in 1:nrow(concs2)){
        ...
        if(concs2[x,3] < 10 & concs2[x,3] > 0){
          concs2[x,35] <- mrbrt[,1][concs2[x,3] > mrbrt[,2] & concs2[x,3] < mrbrt[,3]]
        }else{
